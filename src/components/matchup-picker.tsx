...
export function MatchupPicker({ matchups, decks }: { matchups: LeaderMatchup[]; decks: { leader: string; baseDisplay: string }[] }) {
  ...
  const deckOptions: DeckOption[] = decks
    ...
    .map((d) => {
      ...
      return {
        ...
        label: `${d.leader.split(",")[0]}${setCode ? ` (${setCode})` : ""} · ${d.baseDisplay}`,
      };
    });
